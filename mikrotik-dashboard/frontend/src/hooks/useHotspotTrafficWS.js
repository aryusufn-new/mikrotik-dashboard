...
/**
 * Hook: stream per-user Hotspot traffic from /ws/hotspot-traffic.
 *
 * Returns:
 *   trafficMap  – { [username]: { rx_bps, tx_bps, address, mac_address, uptime, server } }
 *   history     – { [username]: Array<{ t, rx, tx }> }  (last MAX_POINTS samples)
 *   error       – string | null
 */
export default function useHotspotTrafficWS() {
  ...
  const connect = useCallback(() => {
    ...
    let url = `${WS_BASE}/ws/hotspot-traffic?token=${encodeURIComponent(getToken())}`
