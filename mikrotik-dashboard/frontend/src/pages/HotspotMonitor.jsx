...
import { getHotspotActive, getHotspotStats, getHotspotUsers } from '../api'
import { useRouter } from '../context/RouterContext'
import useHotspotTrafficWS from '../hooks/useHotspotTrafficWS'
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-6 z-10 animate-in">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
              ...
              Traffic Real-Time Hotspot: {username}
            </h2>
            <p className="text-xs text-slate-400 mt-0.5">
              Server: {currentTraffic?.server || '-'}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100 text-xs text-slate-400">
          ...
          <div>MAC Address: {currentTraffic?.mac_address || '-'}</div>
...
export default function HotspotMonitor() {
  ...
  const { trafficMap, history, error: wsError } = useHotspotTrafficWS()
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [s, a, u] = await Promise.all([getHotspotStats(), getHotspotActive(), getHotspotUsers()])
        if (!mounted) return
        setStats(s); setActive(a); setUsers(u); setError(null)
      } catch (e) {
  ...
  const activeByUser = useMemo(() => {
    const m = new Map()
    for (const a of active) m.set(a.user, a)
    return m
  }, [active])
  ...
  const rows = useMemo(() => {
    ...
    // 1. Map static local users
    const mergedMap = new Map()
    for (const u of users) {
      mergedMap.set(u.name, {
        name: u.name,
        profile: u.profile,
        disabled: u.disabled,
        online: u.online,
    ...
    // 2. Add active sessions not in local users list (e.g. RADIUS users)
    for (const a of active) {
      if (a.user && !mergedMap.has(a.user)) {
        mergedMap.set(a.user, {
          name: a.user,
          profile: 'RADIUS',
          disabled: false,
          online: true,
    ...
    return mergedList
      .filter((u) => {
        ...
        if (q && !(`${u.name} ${u.profile || ''} ${u.comment || ''}`.toLowerCase().includes(q))) return false
        return true
      })
      .sort((a, b) => Number(b.online) - Number(a.online) || String(a.name ?? '').localeCompare(String(b.name ?? '')))
  }, [users, active, filter, search])
  ...
  return (
    <div>
      <div className="mb-4">
        <h1 className="text-xl sm:text-2xl font-bold text-slate-800">Hotspot Monitor</h1>
        <p className="text-slate-500 text-sm">Status user Hotspot: online & offline</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-4">
        <StatCard title="Total Users" value={stats?.total} color="#a855f7" Icon={Users} />
      ...
      <div className="bg-white rounded-2xl shadow-card p-4 sm:p-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
          <div className="text-sm font-semibold text-slate-700">Hotspot Users</div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-slate-500 text-xs uppercase border-b border-slate-100">
                ...
                <th className="text-left py-2 px-2">MAC Address</th>
            ...
            <tbody>
              {rows.map((u) => {
                const a = activeByUser.get(u.name)
                const liveTraffic = trafficMap[u.name]
                
                return (
                  <tr key={u.name} className="border-b border-slate-50 hover:bg-slate-50 transition">
                    <td className="py-2 px-2 font-medium text-slate-800">{u.name}</td>
                    <td className="py-2 px-2 text-slate-500">{u.profile || '-'}</td>
                    ...
                    <td className="py-2 px-2 text-slate-600">{a?.mac_address || '-'}</td>
                    <td className="py-2 px-2 text-slate-600">{a?.uptime || '-'}</td>
                    <td className="py-2 px-2">
                      {u.online && liveTraffic ? (
                        <div className="flex flex-col sm:flex-row sm:gap-2 text-[11px] leading-tight">
                          <span className="text-brand-purple font-medium">
                            TX: <span className="font-semibold">{formatBps(liveTraffic.rx_bps)}</span>
                          </span>
                          <span className="hidden sm:inline text-slate-300">|</span>
                          <span className="text-brand-pink font-medium">
                            RX: <span className="font-semibold">{formatBps(liveTraffic.tx_bps)}</span>
                          </span>
                        </div>
                      ) : (
                    ...
                    <td className="py-2 px-2 text-slate-500">{u.comment || '-'}</td>
